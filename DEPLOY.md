...
- Cron backups via `deploy/scripts/run-platform-backup.sh` (Docker volume +
  `scripts/backup/snapshot-platform.mjs`); offsite via operator PC download of
  the nightly stamp (`deploy/scripts/pull-platform-backup.sh`), optional
  `BACKUP_S3_*` later ([deploy/hostinger.md](deploy/hostinger.md) §7)
