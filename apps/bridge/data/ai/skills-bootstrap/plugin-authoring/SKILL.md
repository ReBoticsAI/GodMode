...
10. For public Community packs: Sell → **Submit to Community catalog** or `MarketplaceCatalog.prepare_submission` / `submit_submission` (static aliases `prepare_community_catalog_submission` / `submit_community_catalog_submission`). Use git/GitHub tools only for plugin development, not catalog shipping.
