...
export const AUTOMATION_SPECS: BuiltinSpec[] = [
  ...
  { name: "HookRun", label: "Hook Run", module: "automation", id: "hook_run_read", table: "hook_runs", defaultSort: "created_at", actions: HOOK_RUN_ACTIONS, fields: ["id", "hook_id", "event_id", "status", "detail", ["result_json", "JSON"], "created_at"] },
  { name: "PlatformEvent", label: "Platform Event", module: "automation", id: "platform_event_read", table: "platform_events", scope: "tenant", defaultSort: "created_at", actions: PLATFORM_ACTION_METADATA.PlatformEvent, fields: ["id", "type", "actor_kind", "actor_id", "tenant_id", ["payload_json", "JSON"], "created_at"] },
  ...
  { name: "Hook", label: "Hook", module: "automation", id: "hook_service", table: "hooks", scope: "tenant", scopeColumn: "owner_tenant_id", defaultSort: "updated_at", writable: ["owner_kind", "owner_id", "owner_tenant_id", "name", "enabled", "trigger_kind", "event_type", "schedule_cron", "condition_json", "action_kind", "action_config_json", "require_approval"], required: ["owner_kind", "name", "trigger_kind", "action_kind"], operations: ["list", "get", "create", "update", "delete"], actions: TOGGLE_ACTIONS, fields: ["id", "owner_kind", "owner_id", "owner_tenant_id", "name", ["enabled", "Check"], "trigger_kind", "event_type", "schedule_cron", ["condition_json", "JSON"], "action_kind", ["action_config_json", "JSON"], ["require_approval", "Check"], "created_at", "updated_at", "last_fired_at"] },
];
