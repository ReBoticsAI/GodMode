...
function isManagedPlatformSecret(secret: {
  ...
  return (
    ...
    secret.id.startsWith("cursor-api-key__agent__") ||
    name === "cursor_api_key" ||
    name === "cursor-api-key" ||
...
export const providerCredentialRuntimeAdapter: RecordAdapter = {
  ...
  get(db, def, id, ctx) {
    ...
    if (id === XAI_API_KEY_SECRET_ID) {
      const status = getXaiAuthStatus(db, scope);
      return status.connected
        ? record(def, XAI_API_KEY_SECRET_ID, {
            agent_id: scope,
            kind: "api_key",
            provider: "xai",
            display_name: "xAI Console",
            status: "active",
            masked_token: status.masked ?? "****",
          })
        : null;
  ...
  create(db, def, data, ctx) {
    ...
    if (
      requiredText(data, "provider").toLowerCase() === "xai" ||
      requiredText(data, "provider").toLowerCase() === "x-ai"
    ) {
      upsertXaiApiKey(db, requiredText(data, "api_key"), scope);
      const status = getXaiAuthStatus(db, scope);
      return record(def, XAI_API_KEY_SECRET_ID, {
        agent_id: scope,
        kind: "api_key",
        provider: "xai",
        display_name: "xAI Console",
...
export const modelRuntimeAdapter: RecordAdapter = {
  ...
  actions: {
    async select_model(db, _def, _id, input, ctx) {
      ...
      // Custom xAI console slug when Vault is connected.
      if (!selected) {
        const xaiCustom = /^provider:openai_compatible:xai:(.+)$/.exec(modelId);
        if (xaiCustom?.[1] && isXaiPlatformReady(db)) {
          selected = {
            id: modelId,
            source: "provider",
            label: `xAI · ${xaiCustom[1]}`,
            model: xaiCustom[1],
            provider: "openai_compatible",
            transport: "xai",
          };
