...
export function wrapAllowlistCommand(opts: {
  ...
  return [
    ...
    `trap cleanup EXIT`,
    `/usr/bin/node '${scriptAbs}' '${sockAbs}' ${port} &`,
    `BRIDGE_PID=$!`,
    `i=0`,
    `while [ "$i" -lt 50 ]; do`,
    `  /usr/bin/node -e "const n=require('net');const s=n.connect(${port},'127.0.0.1',()=>{s.end();process.exit(0)});s.on('error',()=>process.exit(1))" 2>/dev/null && break`,
    `  i=$((i+1))`,
    `  sleep 0.05`,
    `done`,
    `/usr/bin/node -e "const n=require('net');const s=n.connect(${port},'127.0.0.1',()=>{s.end();process.exit(0)});s.on('error',()=>process.exit(1))" 2>/dev/null || { echo 'egress bridge failed to listen' >&2; exit 1; }`,
    `/bin/sh -c '${userCmd}'`,
  ].join("\n");
}
