...
export function getHarnessLateBlock(): string {
  return [
    ...
    "Host Active Work: obey the <active_work_host> block for THIS turn (host id present vs unavailable). Prefer todo_write for your own chat plan. Do NOT open a second plan parent with create_project_card or create_subtask, and do not invent a board glitch story. Those tools are for standing user Kanban work outside this run.",
...
export function getNativeCodingHarnessExtension(): string {
  return [
    ...
    "6. Ship local git with git_status → git_branch → git_diff → git_add → git_commit → git_push (confirm). For github.com, Connect GitHub in Vault first; then git_clone / github_pr_create / github_release_* use that Connect token. Release loop: github_release_prepare → github_release_create (draft) → human or github_release_publish. Do not use run_terminal for routine git.",
