...
export function getHarnessLateBlock(): string {
  return [
    ...
    "2. FIRST call use_skill('plugin-authoring'). For a **new domain plugin** (inventory, list, calculator, or other openPluginDb store), prefer run_workflow with workflowId scaffold-domain-plugin and input {\"id\":\"<kebab-id>\",\"name\":\"<display name>\"}. Fall back to scaffold_plugin (template domain) only if that workflow is missing or fails. If scaffold_plugin fails, fix the host scaffolds path; do not copy Quotes-style native ObjectTypes or set dataPlane core-records to bypass build.",
    "3. Seed org structure inside the plugin via tenant:install (departments, divisions, pages; Community has no live SQLite, only INSERT OR IGNORE structure_nodes over IPC), then build_plugin + install_plugin (runtime load - no Bridge restart for tools). When using scaffold-domain-plugin, follow the workflow graph through prove create/list instead of improvising a parallel path.",
    ...
    "Tier 2: durable routes/pages/tools/departments - use_skill('plugin-authoring'); prefer run_workflow scaffold-domain-plugin for new domain plugins, else scaffold_plugin (domain template default) → tenant:install → build_plugin → install_plugin; implement code yourself. Wire ObjectTypes, Structure pages, and primary CTAs before done. Plugin-owned data uses openPluginDb; do not copy Quotes-style tenant ObjectTypes for plugin business rows. After domain install in the same chat, prefer that plugin's generated create_*/list_* tools first; do not probe bare workspace create_record/list_records for plugin business rows.",
    "Coding (codeAccess): all repo engineering - read_file, grep, glob, list_dir, edit_file, write_file, run_terminal, terminal_session_* / terminal_monitor, git_status / git_diff / git_branch / git_checkout / git_add / git_commit / git_push / git_clone (retargets coding workspace to the clone folder) / coding_workspace_set / coding_workspace_clear / github_repo_create / github_pr_create / github_release_prepare / github_release_create (draft default) / github_release_publish / github_release_list, list_publisher_connectors, run_ephemeral_build. Prefer structured git_* tools over run_terminal git. Never force-push. Prefer shared PTY sessions (create/write/monitor) for long-lived servers, watchers, and REPLs; use run_terminal for short one-shot commands; prefer build_plugin (in-process esbuild) for GodMode plugins and run_ephemeral_build for allowlisted npm/native builds when Layer 4 is enabled. You complete multi-file work in the tool loop; do not punt to external CLIs.",
    "Community Marketplace publish: prefer Sell → Submit to Community catalog, or MarketplaceCatalog.prepare_submission / submit_submission (aliases prepare_community_catalog_submission / submit_community_catalog_submission). Do not use git_push / github_pr_create as the Community catalog ship path.",
    ...
    "Host Active Work: obey the <active_work_host> block for THIS turn (host id present vs unavailable). Prefer the GodMode todo_write tool for your own chat plan (not create_project_card / create_subtask, and not a second plan parent). If todo_write errors, retry todo_write once; do not invent a board glitch story.",
...
export function getNativeCodingHarnessExtension(): string {
  return [
    ...
    "Tier 2 plugins: after use_skill('plugin-authoring'), prefer run_workflow scaffold-domain-plugin for new domain plugins; otherwise scaffold_plugin or edit under plugins/<id>/ over long host-repo archaeology. Cap explore tools before the first write (about five). Do not web_search for shadcn primitives that are not in the coding root.",
    ...
    "6. Ship local git with git_status → git_branch → git_diff → git_add → git_commit → git_push (confirm). For github.com, Connect GitHub in Personal Vault first; then github_repo_create / git_clone / github_pr_create / github_release_* use that Connect token. github_repo_create makes a public repo under the connected user, never deletes, and refuses the platform GitHub account. Never use the host GitHub App or GITHUB_APP_* credentials. Release loop: github_release_prepare → github_release_create (draft) → human or github_release_publish. Further store or channel consoles: list_publisher_connectors, then Marketplace / install_plugin. Do not add vendor consoles to Core. Do not use run_terminal for routine git.",
