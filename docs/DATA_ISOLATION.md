...
| **1** | Data Router facade under kernel (in-process strangler) | **Shipped for ChatSession/ChatMessage reads:** `listRecords` / `getRecord` return JSON digests via `apps/bridge/src/services/data-router` (flag `DATA_ROUTER_CHAT_READS`, default on). SoR still workspace SQLite; query/report steps are in-process. | #778 |
...
Chat Session and Chat Message **reads** (HTTP Record API and agent `list_records` / `get_record`) go through the Data Router when `DATA_ROUTER_CHAT_READS` is on (default). The Router runs an in-process query step (`listRecordsDirect` / `getRecordDirect`) then a report step that JSON-clones digests so consumers never receive an `AppDatabase`. Writes, SSE/`/ws/chat`, and other ObjectTypes are unchanged. Set `DATA_ROUTER_CHAT_READS=0` for the legacy direct path.
