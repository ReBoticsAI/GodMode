...
Layers 1 and 3 are a **production-minded** boundary for shared hub/SaaS coding: tenant path isolation plus an OS filesystem jail for `run_terminal` and coding helper spawns (`rg`, `tsc`, tenant `git`). They are **not** bank-grade or Cursor Cloud VM isolation.

Still open or weaker by design: Layer 4 ephemeral build VMs, non-HTTP egress (SSH), and interactive coding terminal UI (#148).
