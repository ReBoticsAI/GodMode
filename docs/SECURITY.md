...
| Hub/SaaS ephemeral builds | Host Docker abuse; cross-tenant bind mounts | Layer 4 (#164): host **build supervisor** owns `docker.sock` (never Bridge or tenant jail). Bridge calls localhost bearer HTTP (`CODING_BUILD_SUPERVISOR_URL` / `TOKEN`) like the update supervisor. Ephemeral `docker run --rm` binds only `tenant-workspaces/<tenantId>`; command allowlist (`npm ci` / install / run build / test / typecheck); network `none` by default. Opt-in via `CODING_BUILD_MODE=ephemeral` (fail closed if URL/token missing). Default `build_plugin` stays in-process esbuild |
...
**Layer 4** (optional): ephemeral npm builds via a host build supervisor (`deploy/build-supervisor/`). Residual risks: the supervisor is a privileged Docker client on the host; operators must keep its port loopback-only and rotate `CODING_BUILD_SUPERVISOR_TOKEN`. Non-HTTP egress (SSH) remains out of scope.
