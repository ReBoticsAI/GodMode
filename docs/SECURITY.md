...
**Layer 4** (optional): ephemeral npm builds via a host build supervisor (`deploy/build-supervisor/`). Default build network is `none`. `allowlist` uses a Docker `--internal` network so containers have **no route to the public internet**; only the host CONNECT proxy (via `host.docker.internal`) is reachable, and host allowlisting still applies on CONNECT. Residual risks: the supervisor is a privileged Docker client on the host; operators must keep supervisor and egress-proxy ports loopback-only and rotate `CODING_BUILD_SUPERVISOR_TOKEN`; a same-named non-internal Docker network fails closed until removed. Non-HTTP egress (SSH) and full `shared` build networks remain out of scope.
