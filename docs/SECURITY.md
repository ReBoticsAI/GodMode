...
Layers 1 and 3 are a **production-minded** boundary for shared hub/SaaS coding: tenant path isolation plus an OS filesystem jail for `run_terminal`, shared PTY sessions, coding helper spawns (`rg`, `tsc`, tenant `git`), and the human Coding Terminal. They are **not** bank-grade or Cursor Cloud VM isolation.

Still open or weaker by design: Layer 4 ephemeral build VMs and non-HTTP egress (SSH). Shared interactive PTY (xterm / node-pty + agent Monitor) ships as #162.
