...
| Hub/SaaS ephemeral builds | Host Docker abuse; cross-tenant bind mounts; registry egress | Layer 4 (#164 / #167): host **build supervisor** owns `docker.sock` (never Bridge or tenant jail). Bridge calls localhost bearer HTTP (`CODING_BUILD_SUPERVISOR_URL` / `TOKEN`) like the update supervisor. Ephemeral `docker run --rm` binds only `tenant-workspaces/<tenantId>`; command allowlist (`npm ci` / install / run build / test / typecheck); network `none` by default. Opt-in `CODING_BUILD_NET=allowlist` uses a supervisor CONNECT proxy (ports 80/443) plus container `HTTP(S)_PROXY` for npm/git hosts (`CODING_BUILD_EGRESS_HOSTS` or terminal egress defaults). Opt-in via `CODING_BUILD_MODE=ephemeral` (fail closed if URL/token missing). Default `build_plugin` stays in-process esbuild |

### SaaS coding isolation posture (#112)

Layers 1–3 (plus shared PTY #162) are a **production-minded** boundary for shared hub/SaaS coding: tenant path isolation, OS filesystem jail for terminal/helpers, and kernel-enforced egress allowlist. They are **not** bank-grade or Cursor Cloud VM isolation.

**Layer 4** (optional): ephemeral npm builds via a host build supervisor (`deploy/build-supervisor/`). Default build network is `none`. `allowlist` is proxy-mediated (not kernel netns like Layer 3): tools that ignore `HTTP_PROXY` can still use the container bridge. Residual risks: the supervisor is a privileged Docker client on the host; operators must keep supervisor and egress-proxy ports loopback-only and rotate `CODING_BUILD_SUPERVISOR_TOKEN`. Non-HTTP egress (SSH) and full `shared` build networks remain out of scope.
