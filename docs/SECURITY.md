...
| Hub/SaaS agent worktrees | Accidental edits to live tenant plugins during iterative work | Layer 2 (partial): Bridge-owned git worktrees under `{tenant}/.worktrees/<slug>`; coding tools and `scaffold_plugin` honor `agent.config.workspace`. Promote/merge UX still follow-up |
| Hub/SaaS `run_terminal` | Shell escape past tenant root; host/Docker abuse | Layer 3: bubblewrap FS jail on hub/client Linux (`CODING_TERMINAL_SANDBOX=required`). **Fail closed:** if `bwrap` is missing or the probe fails, `run_terminal` refuses (no unsandboxed fallback). SaaS/hub default `CODING_TERMINAL_NET=none` (`--unshare-net`); set `shared` only when npm/git egress is required. Docker hosts typically need `cap_add: [SYS_ADMIN, NET_ADMIN]` plus `security_opt: [seccomp:unconfined, apparmor:unconfined]` so namespaces work; that is a host privilege tradeoff, not free security. Hostname egress allowlist and interactive shell (#148) remain gated |

### SaaS coding isolation posture (#112)

Layers 1 and 3 are a **production-minded** boundary for shared hub/SaaS coding: tenant path isolation plus an OS filesystem jail for `run_terminal`. They are **not** bank-grade or Cursor Cloud VM isolation.

Still open or weaker by design: Layer 2 promote/merge UX, Layer 4 ephemeral build VMs, hostname egress allowlist, helper spawns (`tsc` / `rg` / `git`) outside the terminal jail, and interactive coding terminal UI (#148).
