...
| Installed plugin code in Bridge process | Malicious plugin APIs against tenant data after install | Distinct from coding jail (#112) and from Marketplace intake CI. Network egress via `host.externalFetch` and tool/record registration/kernel access are deny-by-default for Official/Community (#290 / #303). Kill switches (#96). True plugin process/container sandbox is optional later |
