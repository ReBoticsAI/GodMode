...
Still open or weaker by design: Layer 4 ephemeral build VMs, kernel-enforced egress (iptables/netns) beyond the proxy allowlist, helper spawns (`tsc` / `rg` / `git`) outside the terminal jail, and interactive coding terminal UI (#148).
