...
On GodMode Cloud, **GodMode open-source bug (GitHub)** creates an issue on `ReBoticsAI/GodMode` through the platform GitHub App install (same App as Connect). If the App is unavailable, the UI falls back to opening GitHub’s `issues/new` form. Do not put secrets or operator PII in the issue body.
