...
Kanban parity epic #259 (Done); GitHub App epic #266 (Done). Board filter / search / sort (#276), lifecycle polish (#278), swimlanes / group-by (#274), and Project field parity leftovers (#277) are shipped. Issue comments + soft sync (#293) and Issue timeline activity in the card sheet are shipped. Project Status moves enrich from → to via GraphQL when the token can read Project timeline events; if GraphQL enrichment fails, the sheet still shows the REST fallback line.

### Remaining gaps

- **PR review threads / review comments** are not shown in the card Activity feed (Issue comments API + Issue timeline only). Use the GitHub PR page for review discussions.
- Timeline does not merge Issue comments into one chronological feed with activity lines (comments stay in their own list).
- App / token scopes must allow reading the Issue timeline and Project V2 fields; otherwise enrichment falls back to generic Project move copy.
