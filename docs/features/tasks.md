...
5. Linked boards **poll** GitHub in the background (default about every 3 minutes) and, with a GitHub App, also receive **Projects v2 item webhooks**. The toolbar shows last success, in-progress, and last error. Set `GITHUB_PROJECTS_SYNC_POLL_MS` / `GITHUB_PROJECTS_SYNC_POLL_ENABLED=0` on the Bridge if needed.
